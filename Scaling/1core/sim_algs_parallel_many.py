...
def update_event_list(rank, MT_list,event_list,t,pevent_list,del_mt,last_result,policy,cross_data, rewind):
    ...
    if t==0 or rewind: #calculate all possible pairs
        ...
        if policy == 'comm': #if there's been a communication REDUNDANT, ONLY IN CASE REWIND GOES TO 0
        ...
        if policy == 'comm': #if there's been a communication
            ...
            if policy =='cross_in': #crossing in from left or right
                event_list.pop(0)
                new_idx = [last_result]
            ...
            elif policy in ['left','right', 'top', 'bottom']: #these mts have entered a new processor
                '''PARALLEL EDIT'''
                new_idx = [last_result]
                mt_index= [p for p in mt_index if ((p != last_result) \
                and check_region(last_result,p,MT_list,t,r))] #for comparing new MT
...
def undo_update(rank, mt_list, pevent_list, event_list, del_mt):
    ...
    if policy =='1hit2' or policy=='2hit1':
        ...
        if len(mt_list2) == 0:
            print([mt.number for mt in mt_list])
            print('CANNOT FIND MT IN LIST', rank,t)
            print(event)
            assert False
...
if __name__ == '__main__':
    ...
    while t*conv< T:
        ...
        policy = Result[2]
        if Result[3]*conv > 90:
            plot_snap(0, mt_list, t,i,dest='./', save=True)
